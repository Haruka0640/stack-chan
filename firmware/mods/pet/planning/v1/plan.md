...
CoreS3 ではタッチ位置が分かりづらいため、タッチ開始位置に軽量な波紋 UI を出す。
...
* タッチ開始時の `x`, `y` を `TouchRipple` に保存する
* スワイプ中は最新の `x`, `y` に波紋を追従させる
* スワイプ中は 400ms を超えても波紋を消さない
* 400ms かけて円を広げる
* 顔描画の最後に波紋を描く
* 波紋は白ではなく、少し灰色にする
* 複数波紋には対応しない
* 表情選択、サーボ、PetEvent の処理には影響させない
...
CoreS3 のタッチ開始位置に `TouchRipple` を表示する。

* 1つの `TouchRipple` 状態だけを持つ
* スワイプ中は最新座標に追従する
* スワイプ中は表示を維持する
* 400ms で円を広げる
* 白ではなく少し灰色で描く
* 顔・エフェクトの後に描画する
* 表情・サーボ・イベント処理とは独立させる
