...
- Status: `[ ]`
- Task: `V2-P0-01`
- Notes: v2 要件をタスク化した段階。次は現在の v1 由来実装を v2 の型・状態へ置き換える。
...
- [ ] `V2-P0-01` 現在の v1 由来実装の削除範囲を確認する。
...
- [ ] `V2-P1-03` `Event` と `Reaction` の型を定義する。
...
- [ ] `V2-P1-04` v2 用 runtime state を作成する。
...
- [ ] `V2-P2-01` `pushEvent(EventType type)` を実装する。
  - Acceptance: 現在時刻付きの `Event` がキューに積まれる。
  - Verification: トレースで `EVENT_TAP` などの積み込みが確認できる。
- [ ] `V2-P2-02` `isUserStimulus(EventType type)` を実装する。
...
- [ ] `V2-P2-03` `processEventQueue()` を実装する。
...
- [ ] `V2-P6-01` `mod.ts` を v2 メインループ構成へ更新する。
